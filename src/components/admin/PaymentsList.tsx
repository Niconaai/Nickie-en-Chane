...
export default function PaymentsList({ payments, families, onUpdatePayment }: PaymentsListProps) {
  // NOTE: The getFamilyName function has been removed as it is no longer needed.
  ...
  const getPaymentMethodText = (method: string) => {
    return method === 'yoco' ? 'Kaart Betaling' : 'EFT';
  };
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="divide-y divide-gray-200">
        {payments.map((payment) => {
          const family = families.find(f => f.id === payment.family_id);
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <h3 className="text-lg font-medium text-gray-900">
                    {/* Use the family object directly to get the name. */}
                    {family?.family_name || 'Onbekend'}
                  </h3>
                  ...
                  {/* Correctly access deposit_option from the found family object. */}
                  {family?.deposit_option && (
                    <div className="text-sm text-gray-600 mt-1">
                      Deposito opsie: {family.deposit_option === 'gift' ? 'Geskenk' : 'Terugbetaling'}
                    </div>
                  )}
                </div>
                ...
                <div className="flex space-x-2">
                  ...
                  {/* This button logic would also need the family object if it were to be implemented */}
                </div>
              </div>
